...
async def test_panel_registration_uses_current_schema_cache_key() -> None:
    ...
    assert panel_module.PANEL_SCHEMA_VERSION == 25
    register_panel.assert_awaited_once()
    assert register_panel.await_args.kwargs["module_url"] == (
        "/speedport_smart_frontend/speedport-smart-panel.js?schema=25"
    )
    assert register_panel.await_args.kwargs["config"] == {"schema_version": 25}
