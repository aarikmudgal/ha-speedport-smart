...
export function renderTrafficHistory(snapshot, {language = "en", title, downloadLabel, uploadLabel} = {}) {
  ...
  const legends = SERIES.map((key) => {
    ...
    const description = available ? `${readout} Mbit/s` : words.stale;
    ...
    return `<${tag}${attributes}><span>${escape(labels[key])}</span><strong><span class="sp-traffic-value">${escape(readout)}</span><small>Mbit/s</small></strong>${available ? "" : `<small>${words.stale}</small>`}</${tag}>`;
  }).join("");
